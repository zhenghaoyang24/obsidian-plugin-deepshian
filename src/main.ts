...
export default class DshBridgePlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.addCommand({
      id: "open-dsh-chat",
      name: t("打开 DSH 对话", "Open DSH chat"),
      callback: open,
    });
    this.addCommand({
      id: "new-dsh-chat",
  ...
  checkProfile(): void {
    ...
    if (detectProfile(name)) {
      try {
        ...
        new Notice(
          t(
            "DeepShian: 桥接 profile 修复失败 — ",
            "DeepShian: failed to repair bridge profile — ",
          ) + String(err),
        );
      }
      return;
    }
    new ProfileInstallModal(this.app, name, this.settings.command, () =>
      this.restartBridge(),
    ).open();
  }
  ...
  async loadSettings(): Promise<void> {
    const data = (await this.loadData()) as Partial<DshSettings> | null;
